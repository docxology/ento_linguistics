...
Current pipeline run generates **13 figures** (all sourced from real data):
...
| Full-text layer | `data/fulltexts/fulltexts.json` + `output/data/fulltext_analysis.json` | PMC Open Access full texts (count: `python3 -c "import json; print(len(json.load(open('data/fulltexts/fulltexts.json'))))"`); analysis artifact regenerated by the full-text stage in `src/visualization/manuscript_figures.py` (bound configurable via `FULLTEXT_ANALYSIS_LIMIT`, default 500); rendered via `FULLTEXT_*` tokens |
...
# when data/fulltexts/fulltexts.json exists, the full-text stage additionally
# regenerates output/data/fulltext_analysis.json and fulltext_analysis.png)
