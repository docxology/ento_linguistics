...
- **`src/pipeline/fulltext_pipeline.py`** (full-text parallel layer): applies the same frozen statistics schema to the PMC Open Access full-text corpus (`data/fulltexts/fulltexts.json`) via the shared `build_statistical_analysis` machinery, adding the parallel-layer markers (`layer`, `n_documents`, per-document `token_count`, `domain_term_counts`). Orchestrated by the full-text stage in `src/visualization/manuscript_figures.py::main()` (after the statistics stage; failure warns and continues; bound configurable via `FULLTEXT_ANALYSIS_LIMIT`, default 500), writing `output/data/fulltext_analysis.json` and rendering `fulltext_analysis.png` (registered as `fig:fulltext_analysis`). Clean-slate safe: the stage regenerates the artifact rather than relying on a prior file.
