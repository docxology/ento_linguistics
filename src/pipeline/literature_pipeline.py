...
Orchestrates the complete analysis workflow, from literature mining through
terminology extraction, conceptual mapping, and visualization. Invoked via
the thin orchestrator ``scripts/_literature_analysis_pipeline.py``.
"""
...
class LiteratureAnalysisPipeline:
    ...
    def run_complete_analysis(
        ...
        text processing, terminology extraction, conceptual mapping, domain analysis,
        and visualization generation.

        Args:
            max_publications: Maximum number of publications to collect and analyze (must be > 0)
        ...
        Returns:
            ...
            Each stage contains specific results (publications found, terms extracted, etc.)

        Raises:
            ValueError: If max_publications is not positive
            FileNotFoundError: If cached corpus file exists but cannot be loaded
            RuntimeError: If any pipeline stage fails during execution

        Example:
            >>> pipeline = LiteratureAnalysisPipeline()
            >>> results = pipeline.run_complete_analysis(max_publications=1000)
            >>> print(f"Analyzed {results['stages']['literature_collection']['publications_found']} publications")
        """
    ...
    def _collect_literature(
        ...
        Args:
            max_publications: Maximum publications to collect (must be > 0)
            use_cached: Whether to use cached data if available

        Returns:
            LiteratureCorpus object containing collected publications

        Raises:
            ValueError: If max_publications is not positive
            FileNotFoundError: If cached corpus file exists but cannot be loaded
            RuntimeError: If literature mining fails
        """
        # Validate inputs
    ...
    def _generate_visualizations(
        ...
        try:
            ...
            logger.warning(f"Failed to register terminology_network figure: {e}")
...
def main() -> None:
    """Main entry point for the Ento-Linguistic literature analysis pipeline.

    Command-line interface for running the complete analysis pipeline.
    Supports configuration of publication limits, caching, and verbosity.

    Example:
        $ python scripts/_literature_analysis_pipeline.py --max-publications 1000 --verbose
        $ python scripts/_literature_analysis_pipeline.py --no-cache --output-dir /custom/path
    """
