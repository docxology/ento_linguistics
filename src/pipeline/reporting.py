...
class _FallbackErrorAggregator:
    """Local fallback when infrastructure is not available."""
    ...
    def get_summary(self):
        return {"total_errors": len(self._errors), "errors": self._errors}
...
def get_error_aggregator():
    ...
    Returns:
        ...
        Falls back to a local implementation if infrastructure is not available.
    """
    if _check_infrastructure():
        ErrorAggregator = _get_infra_ErrorAggregator()
        return ErrorAggregator()
    return _FallbackErrorAggregator()
...
def generate_pipeline_report(*args, **kwargs):
    ...
    if not _check_infrastructure():
        return {"status": "skipped", "reason": "infrastructure not available"}
    try:
        func = _get_infra_generate_pipeline_report()
    except ImportError:
        return {"status": "skipped", "reason": "infrastructure pipeline_reporter not available"}
...
"""
    ...
    def save_structured_report(
        ...
        """
        if not _check_infrastructure():
            # Fall back to a simple JSON save when infrastructure is unavailable
            import json
            report_path = self.output_dir / f"{stage_name}_report.json"
            with open(report_path, "w") as f:
                json.dump(results, f, indent=2, default=str)
            return {"json": report_path}

        error_agg = get_error_aggregator()
        gen_report = _get_infra_generate_pipeline_report()
        report = gen_report(
            stage_results=[
        ...
        sav_report = _get_infra_save_pipeline_report()
        return sav_report(report, self.output_dir)
...
# Re-export infrastructure functions and local classes for API consistency
__all__ = [
    "generate_pipeline_report",
