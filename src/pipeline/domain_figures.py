"""Domain figure generation for Ento-Linguistic research.

Generates publication-quality figures for each Ento-Linguistic domain,
visualizing terminology patterns, ambiguities, and conceptual relationships.
Invoked via the thin orchestrator ``scripts/_generate_domain_figures.py``.
"""
...
class DomainFigureGenerator:
    ...
    def _generate_domain_network(
        ...
        label_map = {term: term for term in important_terms}
        nx.draw_networkx_labels(G, pos, labels=label_map, font_size=8, ax=ax)
    ...
    def _generate_comparative_figures(
        ...
        if len(successful_domains) > 1:
            ...
            if domain_data:
                self.visualizer.create_domain_comparison_plot(
