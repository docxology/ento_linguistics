"""Validation framework for Ento-Linguistic data integrity.

This module provides validation checks for terminology extraction results,
corpus consistency, and analysis reproducibility.
"""
...
class ValidationFramework:
    ...
    def validate_reproducibility(
        ...
        message = (
            ...
            if is_valid
            else f"Differences found: {', '.join(differences)}"
        )
    ...
    def detect_anomalies(
        self, values: np.ndarray, method: str = "iqr", threshold: float = 3.0
    ) -> ValidationResult:
    ...
    def validate_quality_metrics(
        self, metrics: Dict[str, float], expected_ranges: Dict[str, Tuple[float, float]]
    ...
    def validate_markdown_dir(self, manuscript_dir: str) -> ValidationResult:
        ...
        result_dict = validate_markdown(manuscript_dir, strict=False)
        # Treat validation errors (e.g. missing infrastructure) as failures
        if result_dict.get("status") == "error":
            result = ValidationResult(
                is_valid=False,
                check_name="markdown_validation",
                message=f"Markdown validation error: {result_dict.get('error', 'unknown')}",
                details={"problems": [], "validation_result": result_dict},
                severity="warning",
            )
            self.validation_results.append(result)
            return result
        problems = result_dict.get("issues", [])
        ...
        result = ValidationResult(
            ...
            message=(
                "No markdown issues"
                if is_valid
                else f"{len(problems)} markdown issues found"
            ),
            details={"problems": problems, "validation_result": result_dict},
    ...
    def validate_figure_registry(
        self, registry_path: str, manuscript_dir: str = "."
    ) -> ValidationResult:
