...
This module provides publication-quality visualization functions for conceptual
mappings, terminology networks, and domain relationships. It is the primary
visualization engine for the Ento-Linguistic research pipeline.

**Module Organisation** (file is ~1,670 lines):

    ConceptVisualizer                               [L32]
    ├── visualize_concept_map()                     [L58]    — NetworkX concept map
    ├── _get_concept_color()                        [L235]   — domain → colour lookup
    ├── _add_domain_legend()                        [L254]   — shared legend helper
    ├── visualize_terminology_network()             [L276]   — term co-occurrence graph
    ├── _get_term_color()                           [L389]   — term → colour lookup
    ├── create_domain_comparison_plot()             [L407]   — 2×3 bar-chart grid
    ├── create_domain_overview_grid()               [L570]   — 6-panel top-10 bars
    ├── create_domain_patterns_grid()               [L698]   — 6-panel POS donuts
    ├── visualize_concept_hierarchy()               [L821]   — centrality + scatter
    ├── create_entropy_distribution()               [L943]   — entropy violin plot
    └── create_anova_results_figure()               [L1100]  — ANOVA heatmap + bars
"""
...
class ConceptVisualizer:
    ...
    def visualize_terminology_network(
        ...
        # Add nodes (terms)
        term_items = terms.items() if isinstance(terms, dict) else terms
        for term_name, term_info in term_items:
            if isinstance(term_info, dict):
                domains = term_info.get("domains", [])
