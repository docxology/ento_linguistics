...
class Term:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Term":
        ...
        Handles graceful defaults for dicts without semantic_entropy.
        """
        # Ensure semantic_entropy has a default for old data
        if "semantic_entropy" not in data:
            data = {**data, "semantic_entropy": 0.0}
        return cls(**data)
...
class TerminologyExtractor:
    ...
    def extract_terms(
        ...
        Args:
            texts: List of input texts (must contain at least one non-empty string)
            min_frequency: Minimum frequency for term inclusion (must be >= 1)
    ...
    def _is_candidate_term(self, token: str) -> bool:
        ...
        # Covers core terminology across all six Ento-Linguistic domains
        scientific_words = {
            # General biology / entomology
            "eusocial",
            "colony",
            "behavior",
            "foraging",
    ...
    def _extract_term_contexts_efficient(
        ...
        Args:
            term: Term to extract contexts for
            text_contexts: List of (text, tokens) pairs
            positions: List of (text_idx, token_pos) tuples
            window_size: Context window size in words
        """
        for text_idx, token_pos in positions:
            text, tokens = text_contexts[text_idx]
