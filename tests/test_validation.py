...
class TestValidationResult:
    ...
    def test_result_creation(self):
        ...
        result = ValidationResult(
            is_valid=True, check_name="test_check", message="Test message"
        )
    ...
    def test_to_dict(self):
        ...
        result = ValidationResult(
            is_valid=False, check_name="test", message="Error", severity="error"
        )
        data = result.to_dict()
...
class TestValidationFramework:
    ...
    def test_validate_sanity(self):
        ...
        validator = ValidationFramework()
        result = validator.validate_sanity(
            5.0, "test_value", expected_order_of_magnitude=1.0
        )
        assert result.is_valid is True
    ...
    def test_validate_sanity_order_of_magnitude_violation(self):
        ...
        result = validator.validate_sanity(
            1e-10, "test_value", expected_order_of_magnitude=1.0
        )
        assert result.is_valid is False
    ...
    def test_validate_quality_metrics(self):
        ...
        expected_ranges = {"accuracy": (0.8, 1.0), "precision": (0.7, 1.0)}
    ...
    def test_generate_validation_report(self):
        ...
        validator.validate_bounds(
            np.array([1, 2, 3]), "test", min_value=0, max_value=10
        )
...
"""
        ...
        assert result.is_valid is False  # Should detect issues or report error
        # When infrastructure is available, specific problems are reported;
        # when missing, the validation error itself causes is_valid=False
        validation_result = result.details.get("validation_result", {})
    ...
    def test_validate_figure_registry(self, tmp_path):
        ...
        registry_file.write_text(
            '{"figures": {"fig1": {"path": "test.png", "caption": "Test"}}}'
        )
    ...
    def test_validate_outputs_success(self, tmp_path):
        """Test output directory validation with valid structure."""
        framework = ValidationFramework()
    ...
    def test_validate_outputs_error_handling(self, tmp_path):
