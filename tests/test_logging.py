...
class TestLogSection:
    ...
    def test_basic_section(self, caplog):
        """Test basic section divider output."""
        from core.logging import log_section

        logger = get_logger("section_basic_test")
        with caplog.at_level(logging.INFO):
            log_section("Term Extraction", logger=logger)
        assert "Term Extraction" in caplog.text
        # Check that the Unicode separator is present
        assert "\u2550" in caplog.text

    def test_section_with_elapsed(self, caplog):
        """Test section divider with elapsed time suffix."""
        from core.logging import log_section

        logger = get_logger("section_elapsed_test")
        with caplog.at_level(logging.INFO):
            log_section("Analysis", elapsed=3.21, logger=logger)
        assert "Analysis" in caplog.text
        assert "[3.21 s]" in caplog.text

    def test_section_custom_width(self, caplog):
        """Test section divider with custom width."""
        from core.logging import log_section

        logger = get_logger("section_width_test")
        with caplog.at_level(logging.INFO):
            log_section("Narrow", width=30, logger=logger)
        assert "Narrow" in caplog.text

    def test_section_default_logger(self, caplog):
        """Test section with default logger (None)."""
        from core.logging import log_section

        with caplog.at_level(logging.INFO):
            log_section("Default Section")
        assert "Default Section" in caplog.text
