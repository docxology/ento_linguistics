...
class TestTerminologyExtractionIntegration:
    ...
    def test_all_six_domain_seeds_populated(self) -> None:
        """Every domain seed set must be non-empty."""
        extractor = TerminologyExtractor()
        expected_domains = {
        ...
        assert set(extractor.DOMAIN_SEEDS.keys()) == expected_domains
        for domain, seeds in extractor.DOMAIN_SEEDS.items():
            assert len(seeds) >= 3, f"Domain {domain} has too few seeds"

    def test_cross_domain_terms_detected(self) -> None:
        """Terms like 'queen' should appear in multiple domains."""
        extractor = TerminologyExtractor()
        domains = extractor.classify_term_domains("queen")
        assert len(domains) >= 2, "'queen' should span power_and_labor + sex_and_reproduction"

    def test_extract_terms_preserves_term_metadata(self) -> None:
        """Extracted terms should have lemma, frequency, confidence, and domains."""
        extractor = TerminologyExtractor()
        texts = [
            "The colony exhibits eusocial behavior with foraging workers.",
            "Colony workers forage for resources using eusocial cooperation.",
            "Eusocial colonies have workers that forage and defend the nest.",
        ]
        terms = extractor.extract_terms(texts, min_frequency=2)
        for name, term in terms.items():
            assert term.text, f"Term {name} missing text"
            assert term.lemma, f"Term {name} missing lemma"
            assert term.frequency >= 2, f"Term {name} below min_frequency"
            assert 0.0 <= term.confidence <= 1.0, f"Term {name} confidence out of bounds"

    @pytest.mark.parametrize("domain_term,expected_domain", [
        ("colony", "unit_of_individuality"),
        ("foraging", "behavior_and_identity"),
        ("caste", "power_and_labor"),
        ("queen", "sex_and_reproduction"),
        ("relatedness", "kin_and_relatedness"),
        ("resource", "economics"),
    ])
    def test_domain_classification_parametrized(
        self, domain_term: str, expected_domain: str
    ) -> None:
        """Each known term should classify into its expected domain."""
        extractor = TerminologyExtractor()
        domains = extractor.classify_term_domains(domain_term)
