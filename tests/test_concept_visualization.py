...
class TestConceptVisualizer:
    ...
    def test_get_term_color(self, visualizer: ConceptVisualizer) -> None:
        ...
        # Create a sample graph with node attributes
        G = nx.Graph()
...
class TestConceptVisualizerFallbackPaths:
    ...
    def test_concept_map_empty_graph(self):
        """Empty concept map shows 'No concepts' message."""
        viz = ConceptVisualizer()
        empty_map = ConceptMap()
        fig = viz.visualize_concept_map(empty_map, title="Empty")
        assert isinstance(fig, plt.Figure)
        plt.close(fig)

    def test_concept_map_save_to_file(self):
        """Concept map saved to filepath closes the figure."""
        viz = ConceptVisualizer()
        cmap = ConceptMap()
        c = Concept(
            name="foraging",
            description="food retrieval",
            terms={"scout", "trail"},
        ...
        with tempfile.TemporaryDirectory() as tmp:
            fp = Path(tmp) / "map.png"
            fig = viz.visualize_concept_map(cmap, title="Save", filepath=fp)
            assert fp.exists()
            assert isinstance(fig, plt.Figure)

    def test_anthropomorphic_empty_data(self):
        """Empty anthropomorphic dict triggers the placeholder branch."""
        viz = ConceptVisualizer()
        fig = viz.create_anthropomorphic_analysis_plot({})
        assert isinstance(fig, plt.Figure)
        plt.close(fig)

    def test_anthropomorphic_save_to_file(self):
        """Anthropomorphic plot saved to filepath."""
        viz = ConceptVisualizer()
        data = {"kinship": ["aunt", "uncle", "sister", "brother", "cousin", "mother"]}
        with tempfile.TemporaryDirectory() as tmp:
            fp = Path(tmp) / "anthro.png"
            fig = viz.create_anthropomorphic_analysis_plot(data, filepath=fp)
            assert fp.exists()
            assert isinstance(fig, plt.Figure)

    def test_domain_overview_grid_empty_terms(self):
        """Domain overview grid with empty term dict."""
        viz = ConceptVisualizer()
        domain_data = {
            "unit_of_individuality": {"count": 0},
            "behavior_and_identity": {"count": 0},
            "power_and_labor": {"count": 0},
            "sex_and_reproduction": {"count": 0},
            "kin_and_relatedness": {"count": 0},
            "economics": {"count": 0},
        }
        fig = viz.create_domain_overview_grid(domain_data, terms={})
        assert isinstance(fig, plt.Figure)
        plt.close(fig)

    def test_domain_patterns_grid_empty_terms(self):
        """Domain patterns grid with empty term dict."""
        viz = ConceptVisualizer()
        fig = viz.create_domain_patterns_grid(terms={})
        assert isinstance(fig, plt.Figure)
        plt.close(fig)

    def test_fallback_cooccurrence_empty(self):
        """Fallback co-occurrence plot with no data."""
        viz = ConceptVisualizer()
        fig, ax = plt.subplots()
        viz._create_fallback_cooccurrence_plot({}, ax, "Empty Co-occ")
        plt.close(fig)

    def test_fallback_cooccurrence_with_data(self):
        """Fallback co-occurrence plot with data renders bars."""
        viz = ConceptVisualizer()
        fig, ax = plt.subplots()
        matrix = {
            "queen": {"worker": 5, "colony": 3},
            "worker": {"queen": 5, "brood": 2},
        }
    ...
    def test_get_concept_color_3d_no_domain(self):
        ...
        viz = ConceptVisualizer()
        cmap = ConceptMap()
        c = Concept(name="x", description="d", terms=set(), domains=set())
        cmap.concepts["x"] = c
        color = viz._get_concept_color_3d("x", cmap)
        assert color == "gray"

    def test_get_concept_color_3d_missing_concept(self):
        """_get_concept_color_3d returns gray for unknown concept names."""
        viz = ConceptVisualizer()
        cmap = ConceptMap()
        color = viz._get_concept_color_3d("nonexistent", cmap)
        assert color == "gray"

    def test_domain_overview_grid_with_semantic_entropy(self):
        """Domain overview grid where domain-level semantic_entropy is supplied."""
        viz = ConceptVisualizer()
        from types import SimpleNamespace

        terms = {
            "queen": SimpleNamespace(
                frequency=10, domains=["unit_of_individuality"],
                semantic_entropy=1.5, pos_tags=["NN"],
            ),
            "worker": SimpleNamespace(
                frequency=8, domains=["power_and_labor"],
                semantic_entropy=1.2, pos_tags=["NN"],
            ),
        }
        domain_data = {
            "unit_of_individuality": {"count": 1, "semantic_entropy": 1.5},
            "behavior_and_identity": {"count": 0},
            "power_and_labor": {"count": 1, "semantic_entropy": 1.2},
            "sex_and_reproduction": {"count": 0},
            "kin_and_relatedness": {"count": 0},
            "economics": {"count": 0},
        }
        fig = viz.create_domain_overview_grid(domain_data, terms=terms)
        assert isinstance(fig, plt.Figure)
        plt.close(fig)

    def test_domain_patterns_grid_with_terms(self):
        """Domain patterns grid with real terms produces donut charts."""
        viz = ConceptVisualizer()
        from types import SimpleNamespace
        ...
        terms = {
            ...
            "nurse bee": SimpleNamespace(
                text="nurse bee", frequency=3,
                domains=["behavior_and_identity"],
                pos_tags=[],
            ),
        }
