...
class TestMeasurePersuasiveEffectiveness:
    ...
    def test_nonzero_impact_for_persuasive_content(self):
        ...
        texts = [
            "How do ants decide? Why do colonies cooperate? What mechanism drives behavior?",
            "Colonies process 50% more food (Smith 2021). Workers are 3.5 times faster (Jones 2022).",
            "Like a factory, the colony system operates efficiently. Similar to human organizations.",
            "Research shows significant results (Brown 2020). Evidence confirms (Lee 2019).",
        ]
...
class TestTrackConceptualShifts:
    ...
    def test_nonzero_shifts_with_changing_patterns(self):
        """Texts with different rhetorical patterns should produce non-zero shifts."""
        texts = [
            "Authority citations (Smith 2020) show ant behavior (Jones 2019). Evidence (Brown 2018).",
            "Simple observation of ants walking. No citations here at all.",
        ]
        result = track_conceptual_shifts(texts)
        assert len(result) == 1
        shift_data = list(result.values())[0]
        # At least one pattern should show a non-zero shift
        has_nonzero_shift = any(
            ps["shift_magnitude"] > 0
            for ps in shift_data["pattern_shifts"].values()
        )
        assert has_nonzero_shift, (
            "Changing rhetorical patterns between periods should produce non-zero shifts"
        )
