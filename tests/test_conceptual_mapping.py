...
class TestDomainOverlapRatioDynamic:
    """Test that domain_overlap_ratio is calculated dynamically, not hardcoded."""

    def test_domain_overlap_ratio_uses_union_not_hardcoded(self):
        """domain_overlap_ratio should use len(all_domains) dynamically, not a constant like /6."""
        mapper = ConceptualMapper()
        cm = ConceptMap()

        # Concept with 2 domains
        c1 = Concept(name="concept_a", description="Concept A")
        c1.add_term("term1")
        c1.add_domain("domain1")
        c1.add_domain("domain2")

        # Concept with 3 domains, 1 overlapping with c1
        c2 = Concept(name="concept_b", description="Concept B")
        c2.add_term("term2")
        c2.add_domain("domain2")
        c2.add_domain("domain3")
        c2.add_domain("domain4")

        cm.add_concept(c1)
        cm.add_concept(c2)
        cm.add_relationship("concept_a", "concept_b", 0.5)

        strengths = mapper.quantify_relationship_strength(cm)
        key = ("concept_a", "concept_b")
        assert key in strengths

        # domain overlap = 1 (domain2), union = 4 (domain1..4)
        # ratio should be 1/4 = 0.25, NOT 1/6
        assert strengths[key]["domain_overlap"] == 1
        assert strengths[key]["domain_overlap_ratio"] == pytest.approx(1 / 4, abs=1e-10)

    def test_domain_overlap_ratio_varies_with_domain_count(self):
        """Ratio should change when concepts have different numbers of domains."""
        mapper = ConceptualMapper()

        # Setup 1: 1 domain each, same domain -> ratio = 1/1 = 1.0
        cm1 = ConceptMap()
        c1a = Concept(name="a", description="A")
        c1a.add_domain("d1")
        c1b = Concept(name="b", description="B")
        c1b.add_domain("d1")
        cm1.add_concept(c1a)
        cm1.add_concept(c1b)
        cm1.add_relationship("a", "b", 0.5)
        s1 = mapper.quantify_relationship_strength(cm1)
        ratio1 = s1[("a", "b")]["domain_overlap_ratio"]

        # Setup 2: 3 domains each, 1 overlapping -> ratio = 1/5
        cm2 = ConceptMap()
        c2a = Concept(name="a", description="A")
        c2a.add_domain("d1")
        c2a.add_domain("d2")
        c2a.add_domain("d3")
        c2b = Concept(name="b", description="B")
        c2b.add_domain("d1")
        c2b.add_domain("d4")
        c2b.add_domain("d5")
        cm2.add_concept(c2a)
        cm2.add_concept(c2b)
        cm2.add_relationship("a", "b", 0.5)
        s2 = mapper.quantify_relationship_strength(cm2)
        ratio2 = s2[("a", "b")]["domain_overlap_ratio"]

        assert ratio1 != ratio2, "Ratios should differ with different domain counts"
        assert ratio1 == pytest.approx(1.0, abs=1e-10)
        assert ratio2 == pytest.approx(1 / 5, abs=1e-10)
