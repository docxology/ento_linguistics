...
"""
    ...
    def test_main_function_handles_src_import_failures(self, tmp_path):
        """Test script reports error when src/ analysis modules unavailable."""
        ...
        # analysis.text_analysis etc. are hard requirements).
        # NOTE: If the project is installed in the venv (editable install),
        # analysis modules remain importable via site-packages even with
        # stripped PYTHONPATH, so the script may succeed.
        if result.returncode == 0:
            # Installed package makes analysis importable - this is acceptable
            pass
        else:
            combined_output = result.stdout + result.stderr
