...
The `tests/integration/` directory validates end-to-end functionality of the Ento-Linguistic pipeline. Tests exercise real data flows from corpus loading (`data/corpus/abstracts.json`), terminology extraction and domain classification across the six domains, CACE scoring, semantic entropy computation, conceptual mapping, literature mining (via pytest-httpserver), figure generation, markdown integration, and manuscript validation.

All tests use real computation and data. No mocks. Fixtures in `conftest.py` provide seeded corpus subsets and temporary directories. The `test_integration_pipeline.py` sets up a temporary project structure by copying `infrastructure/` and `src/` and runs validation CLI, glossary generation, and full analysis steps.
...
def _setup_test_project_structure(tmp_path: Path, test_name: str) -> Path:
    """Creates temporary structure with infrastructure/ copy, src/, scripts/, data/, output/, manuscript/."""
    # ... (copies from repo_root and project_root)
    return test_root
```

**Core validation flow (excerpt from test_integration_pipeline):**

```python
# Uses real corpus and infrastructure validation
repo_root, cli_script, glossary_script = _get_infrastructure_paths()
result = subprocess.run(
    [sys.executable, "-m", "infrastructure.validation.cli", "markdown", str(manuscript_dir)],
    capture_output=True,
    text=True,
    cwd=str(test_root)
)
# Validates glossary generation via generate_glossary_cli
# Checks figure_registry.json, output_integrity, term extraction results
```

**Tests TerminologyExtractor and ValidationFramework:**

```python
from src.analysis.term_extraction import TerminologyExtractor, Term
from src.core.validation import ValidationFramework, ValidationResult

extractor = TerminologyExtractor()
terms = extractor.extract_terms(sample_texts, min_frequency=2)  # returns dict[str, Term]
# Term has: to_dict(), from_dict(), add_context(), domains list, semantic_entropy
...
### Literature Mining (`test_literature_mining.py`)

Uses `pytest-httpserver` for real HTTP simulation of PubMed/arXiv without external calls:

```python
def test_with_httpserver(self, httpserver, monkeypatch):
    httpserver.expect_request(...).respond_with_data(json_data)
    # Tests PubMedMiner.search(), LiteratureCorpus, load_real_corpus()
```

### Domain and Analysis Tests

- `test_domain_analysis.py`: `analyze_all_domains(terms, texts)` across unit_of_individuality, behavior_identity, power_labor, sex_reproduction, kin_relatedness, economics.
- `test_cace_scoring.py`, `test_semantic_entropy.py`, `test_conceptual_mapping.py`: Validate CACE framework, entropy computation (with KMeans(random_state=42)), network construction.
- `test_markdown_integration.py`, `test_manuscript_preflight.py`: Test `src/core/markdown_integration.py` for \ref{}, figure_registry validation, 98_symbols_glossary.md generation.
- Figure tests (`test_generate_research_figures.py`, `test_concept_visualization.py`): Use `src/visualization/figure_manager.py`, `concept_visualization.py`; validate against `output/figures/figure_registry.json` (11 figures).
...
- [`../AGENTS.md`](../AGENTS.md) - Full test suite overview with test-to-module mapping
- [`../../src/AGENTS.md`](../../src/AGENTS.md) - Source signatures (TerminologyExtractor, ValidationFramework, DataLoader.load_real_corpus)
- [`../../scripts/AGENTS.md`](../../scripts/AGENTS.md) - Thin orchestrators (01_build_corpus.py, 02_generate_figures.py)
- Root [`AGENTS.md`](../../AGENTS.md) - Template standards (no-mocks, thin orchestrator, reproducibility with seed=42)
