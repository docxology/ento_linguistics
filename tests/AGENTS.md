...
The `tests/` directory contains tests for the project-specific scientific code in `src/`. These tests validate the research algorithms, data processing pipelines, and analysis methods that form the core of the research project.

## Test Coverage Requirements

**90% minimum coverage** required for `src/` modules before PDF generation proceeds. This ensures all critical research code paths are validated.
...
├── test_persuasive_analysis.py    # Persuasive analysis
├── test_pipeline_init.py          # Pipeline init
├── test_plots.py                  # Plotting function tests
├── test_quality_report.py         # Quality report
├── test_reporting.py              # Report generation tests
├── test_rhetorical_analysis.py    # Rhetorical analysis
├── test_semantic_entropy.py       # Semantic entropy
├── test_simulation.py             # Simulation engine tests
├── test_statistical_visualization.py # Statistical visualization
├── test_statistics.py             # Statistical analysis tests
├── test_term_extraction.py        # Term extraction
├── test_text_analysis.py           # Text analysis
...
Run `uv run pytest tests/ --cov=src --cov-report=term-missing` for current coverage. **90% minimum** required for `src/` modules. All analysis, core, data, pipeline, and visualization modules have corresponding test files.
