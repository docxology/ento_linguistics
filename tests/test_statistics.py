...
class TestBenjaminiHochbergCorrection:
    ...
    def test_empty_input(self):
        """Empty p-value list returns empty structures."""
        result = benjamini_hochberg_correction([])
        assert result["adjusted_p_values"] == []
        assert result["rejected"] == []
        assert result["n_rejected"] == 0
        assert result["alpha_threshold"] == 0.0

    def test_basic_correction(self):
        """BH correction rejects small p-values and adjusts larger ones."""
        p_vals = [0.01, 0.04, 0.03, 0.2, 0.5]
        result = benjamini_hochberg_correction(p_vals, q=0.05)
        ...
        # The smallest p-value (0.01) should be rejected
    ...
    def test_adjusted_pvalues_bounded(self):
        ...
        p_vals = [0.001, 0.01, 0.05, 0.1, 0.3, 0.5, 0.8, 0.9]
        result = benjamini_hochberg_correction(p_vals, q=0.05)
    ...
    def test_adjusted_pvalues_monotone(self):
        ...
        p_vals = [0.005, 0.011, 0.022, 0.033, 0.044]
        result = benjamini_hochberg_correction(p_vals, q=0.05)
        # Sort by original index, then check that BH adjustments are sensible
        # (smaller raw p-values get smaller or equal adjusted p-values)
        adj = result["adjusted_p_values"]
        sorted_idx = sorted(range(len(p_vals)), key=lambda i: p_vals[i])
        sorted_adj = [adj[i] for i in sorted_idx]
        for i in range(len(sorted_adj) - 1):
            assert sorted_adj[i] <= sorted_adj[i + 1] + 1e-10  # monotone non-decreasing

    def test_single_pvalue(self):
        """Single p-value: adjusted equals original (capped at 1.0)."""
        result = benjamini_hochberg_correction([0.04], q=0.05)
        assert len(result["adjusted_p_values"]) == 1
        assert result["adjusted_p_values"][0] == pytest.approx(0.04, rel=1e-6)

    def test_result_order_preserved(self):
        """Output order must match input order, not rank order."""
        p_vals = [0.3, 0.01, 0.2]  # index 1 is the smallest
        result = benjamini_hochberg_correction(p_vals, q=0.05)
        assert len(result["adjusted_p_values"]) == len(p_vals)
        assert len(result["rejected"]) == len(p_vals)
        # Index 1 (p=0.01) should be rejected
        assert result["rejected"][1] is True


class TestTTestEdgeCases:
    """Edge case tests for t-test functionality."""

    def test_two_sample_zero_variance_different_means(self):
        """Two groups with zero variance but different means should not raise ZeroDivisionError."""
        sample1 = np.array([5.0, 5.0, 5.0])
        sample2 = np.array([3.0, 3.0, 3.0])
        result = t_test(sample1, sample2)
        # Should not crash; t_stat should be inf (means differ, pooled_std is 0)
        assert "t_statistic" in result
        assert "p_value" in result
        assert np.isinf(result["t_statistic"]) or result["t_statistic"] != 0.0

    def test_two_sample_zero_variance_identical_means(self):
        """Two identical groups with zero variance should give t_stat == 0."""
        sample1 = np.array([5.0, 5.0, 5.0])
        sample2 = np.array([5.0, 5.0, 5.0])
        result = t_test(sample1, sample2)
        assert result["t_statistic"] == 0.0
        # p-value should be 1.0 (no difference at all)
        assert result["p_value"] >= 0.99

    @pytest.mark.filterwarnings("ignore:Degrees of freedom <= 0:RuntimeWarning")
    @pytest.mark.filterwarnings("ignore:invalid value encountered:RuntimeWarning")
    def test_two_sample_single_element_group(self):
        """A group with sample size 1 should not crash (n-1 == 0 in Welch df)."""
        sample1 = np.array([10.0])
    ...
    def test_descriptive_stats_uses_sample_std(self):
        """DescriptiveStats.std must use ddof=1 (sample std), not ddof=0 (population std)."""
        data = np.array([2.0, 4.0, 4.0, 4.0, 5.0, 5.0, 7.0, 9.0])
        stats = calculate_descriptive_stats(data)
        expected_sample_std = float(np.std(data, ddof=1))
        population_std = float(np.std(data, ddof=0))
