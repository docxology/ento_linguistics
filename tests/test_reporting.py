...
class TestReportingModuleFunctions:
    """Test module-level reporting functions and infrastructure fallback paths."""

    def test_fallback_error_aggregator_add_and_summary(self):
        """Test _FallbackErrorAggregator.add_error and get_summary directly."""
        from pipeline.reporting import _FallbackErrorAggregator

        agg = _FallbackErrorAggregator()
        assert agg.get_summary() == {"total_errors": 0, "errors": []}
        agg.add_error(code="E001", msg="test error", severity="warning")
        summary = agg.get_summary()
        assert summary["total_errors"] == 1
        assert summary["errors"][0]["code"] == "E001"

    def test_get_error_aggregator_returns_instance(self):
        """Test get_error_aggregator returns a usable aggregator."""
        from pipeline.reporting import get_error_aggregator

        agg = get_error_aggregator()
        assert hasattr(agg, "add_error")
        assert hasattr(agg, "get_summary")
        # ErrorAggregator.add_error requires error_type and message; _FallbackErrorAggregator accepts **kwargs
        try:
            agg.add_error(error_type="test", message="test msg")
        except TypeError:
            # Fallback for _FallbackErrorAggregator which accepts **kwargs
            agg.add_error(msg="test")
        summary = agg.get_summary()
        assert isinstance(summary, dict)

    def test_generate_pipeline_report_fallback(self, monkeypatch):
        """Test generate_pipeline_report returns skip dict when infra unavailable."""
        import pipeline.reporting as rep_module

        monkeypatch.setattr(rep_module, "_HAS_INFRASTRUCTURE", False)
        result = rep_module.generate_pipeline_report()
        assert result.get("status") == "skipped"
        assert "infrastructure" in result.get("reason", "").lower()

    def test_save_pipeline_report_fallback(self, monkeypatch):
        """Test save_pipeline_report returns empty dict when infra unavailable."""
        import pipeline.reporting as rep_module

        monkeypatch.setattr(rep_module, "_HAS_INFRASTRUCTURE", False)
        result = rep_module.save_pipeline_report({"report": "data"}, "/tmp")
        assert result == {}

    def test_save_structured_report_no_infrastructure(self, tmp_path, monkeypatch):
        """Test save_structured_report fallback JSON when infrastructure is absent."""
        import pipeline.reporting as rep_module

        monkeypatch.setattr(rep_module, "_HAS_INFRASTRUCTURE", False)
        gen = rep_module.ReportGenerator(output_dir=str(tmp_path))
        results = {"accuracy": 0.95, "status": "done"}
        paths = gen.save_structured_report("test_stage", results, pipeline_duration=1.0)
    ...
    def test_check_infrastructure_caches_result(self, monkeypatch):
        """Test that _check_infrastructure caches its result after first call."""
        import pipeline.reporting as rep_module

        original = rep_module._HAS_INFRASTRUCTURE
        monkeypatch.setattr(rep_module, "_HAS_INFRASTRUCTURE", None)
        result1 = rep_module._check_infrastructure()
