...
class TestCalculateSemanticEntropy:
    ...
    def test_no_degenerate_uniform_entropy_with_five_contexts(self):
        """Regression: five contexts must NOT produce H = log2(5) ≈ 2.32 for all terms.

        Prior to the fix, n_clusters was set to min(max_clusters, n) which
        equalled 5 when n == max_clusters == 5, placing one context per cluster
        and yielding identical H for every term regardless of actual semantics.
        """
        # Two terms with very different context distributions:
        # "queen" — all contexts about the same reproductive role
        same_sense = [
            "The queen ant lays eggs in the nest.",
            "Queens are the sole reproductive caste.",
            "The queen produces hundreds of eggs daily.",
            "A colony queen mates during the nuptial flight.",
            "Foundress queens establish new colonies alone.",
        ]
        # "colony" — deliberately varied semantic senses
        mixed_sense = [
        ...
        r_same = calculate_semantic_entropy("queen", same_sense, random_state=42)
        r_mixed = calculate_semantic_entropy("colony", mixed_sense, random_state=42)

        # With the fix, k < n, so clusters are non-trivial and entropy
        # should NOT equal the degenerate log2(5) ≈ 2.32 that the old code
        # produced for every five-context term.
        log2_5 = np.log2(5)
        assert abs(r_same.entropy_bits - log2_5) > 0.01
        assert abs(r_mixed.entropy_bits - log2_5) > 0.01
        assert r_same.n_clusters < 5
        assert r_mixed.n_clusters < 5
        # Both should produce meaningful, non-zero entropy
        assert r_same.entropy_bits > 0.0
        assert r_mixed.entropy_bits > 0.0
