...
class TestEvaluateTermCACEEdgeCases:
    """Edge case tests for CACE evaluation."""

    def test_zero_entropy_no_contexts_no_domains(self):
        """Evaluate with minimal inputs should not crash and produce bounded scores."""
        result = evaluate_term_cace(
            term="unknown_term",
    ...
    def test_max_entropy_many_domains(self):
        """High entropy and many domains should still produce bounded scores."""
        result = evaluate_term_cace(
            term="slave",
            semantic_entropy=10.0,
            contexts=["slave labor in the colony"] * 20,
            domains=["power_and_labor", "behavior_and_identity", "economics",
                     "unit_of_individuality", "sex_and_reproduction", "kin_and_relatedness"],
        )
        assert 0.0 <= result.clarity <= 1.0
        assert 0.0 <= result.appropriateness <= 1.0
        assert 0.0 <= result.consistency <= 1.0
    ...
    def test_all_anthropomorphic_terms_bounded(self):
        """Every known anthropomorphic term should produce scores in [0, 1]."""
        for term in ANTHROPOMORPHIC_TERMS:
            result = evaluate_term_cace(
                term=term,
                semantic_entropy=1.5,
                contexts=[f"The {term} in the colony."] * 5,
                domains=["power_and_labor"],
            )
            assert 0.0 <= result.clarity <= 1.0, f"{term} clarity out of bounds"
            assert 0.0 <= result.appropriateness <= 1.0, f"{term} appropriateness out of bounds"
            assert 0.0 <= result.consistency <= 1.0, f"{term} consistency out of bounds"
            assert 0.0 <= result.evolvability <= 1.0, f"{term} evolvability out of bounds"
            assert 0.0 <= result.aggregate <= 1.0, f"{term} aggregate out of bounds"

    def test_aggregate_is_mean_for_extreme_inputs(self):
        """Aggregate should always be the mean of dimensions, even for extreme inputs."""
        result = evaluate_term_cace(
            term="haplodiploidy",
            semantic_entropy=0.0,
            contexts=[],
        ...
        expected = np.mean([result.clarity, result.appropriateness,
                           result.consistency, result.evolvability])
        assert result.aggregate == pytest.approx(expected, abs=1e-6)
...
class TestCACEParametrized:
    """Parametrized CACE tests across all anthropomorphic terms and domains."""

    @pytest.mark.parametrize("term", sorted(ANTHROPOMORPHIC_TERMS))
    @pytest.mark.parametrize("domain", ALL_DOMAINS)
    def test_all_anthropomorphic_terms_bounded_across_domains(self, term, domain):
        """Every anthropomorphic term in every domain must produce scores in [0, 1]."""
        result = evaluate_term_cace(
            term=term,
            semantic_entropy=1.5,
            contexts=[f"The {term} in the colony is important."] * 5,
            domains=[domain],
        )
        assert 0.0 <= result.clarity <= 1.0
        assert 0.0 <= result.appropriateness <= 1.0
    ...
    @pytest.mark.parametrize("term", sorted(ANTHROPOMORPHIC_TERMS))
    def test_anthropomorphic_always_penalized(self, term):
        """Every anthropomorphic term must score below 1.0 on appropriateness."""
        score = score_appropriateness(term)
        assert score < 1.0, f"'{term}' should be penalized but scored {score}"

    @pytest.mark.parametrize("term", [
        "haplodiploidy", "trophallaxis", "stigmergy", "nestmate",
        "superorganism", "brood", "pheromone", "mandible",
    ])
    def test_non_anthropomorphic_terms_score_perfect_appropriateness(self, term):
        """Scientific terms must score 1.0 on appropriateness."""
        score = score_appropriateness(term)
        assert score == 1.0, f"'{term}' should not be penalized but scored {score}"

    @pytest.mark.parametrize("n_domains", [1, 2, 3, 4, 5, 6])
    def test_cross_domain_penalty_monotonic(self, n_domains):
        """More domains should yield equal or lower appropriateness for 'queen'."""
        domains = ALL_DOMAINS[:n_domains]
        score = score_appropriateness("queen", domains=domains)
        assert 0.0 <= score <= 1.0
        if n_domains > 1:
