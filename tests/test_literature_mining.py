...
class TestPubMedMinerParsing:
    """Cover XML parsing, batch error handling, and edge cases."""

    def test_parse_pubmed_summary_no_title(self) -> None:
        """_parse_pubmed_summary returns None when title is missing."""
        miner = PubMedMiner(enable_cache=False)
        result = miner._parse_pubmed_summary({"authors": []})
        assert result is None

    def test_parse_pubmed_summary_with_doi(self) -> None:
        """_parse_pubmed_summary extracts DOI from elocationid."""
        miner = PubMedMiner(enable_cache=False)
        data = {
            "title": "Test Paper",
    ...
    def test_parse_pubmed_summary_no_doi_prefix(self) -> None:
        """elocationid without DOI: prefix is treated as no DOI."""
        miner = PubMedMiner(enable_cache=False)
        data = {"title": "Test", "elocationid": "pii: S1234"}
        pub = miner._parse_pubmed_summary(data)
        assert pub is not None
        assert pub.doi is None

    def test_parse_pubmed_summary_exception(self) -> None:
        """_parse_pubmed_summary returns None on unexpected error."""
        miner = PubMedMiner(enable_cache=False)
        # Cause TypeError by passing a non-dict-like object disguised
        result = miner._parse_pubmed_summary({"title": "ok", "pubdate": 12345})
        # pubdate=12345 will fail re.search (expects str) -- caught by except block
        assert result is None

    def test_fetch_batch_summaries_empty(self) -> None:
        """_fetch_batch_summaries returns empty list for empty input."""
        miner = PubMedMiner(enable_cache=False)
        assert miner._fetch_batch_summaries([]) == []

    def test_fetch_batch_abstracts_empty(self) -> None:
        """_fetch_batch_abstracts returns empty dict for empty input."""
        miner = PubMedMiner(enable_cache=False)
        ...
        <PubmedArticleSet>
                ...
                <MedlineCitation>
                    ...
                    <Article>
                        <Abstract>
                            ...
                            <AbstractText>Main findings here.</AbstractText>
                        </Abstract>
                    </Article>
                </MedlineCitation>
            </PubmedArticle>
            <PubmedArticle>
                <MedlineCitation>
                    <Article>
                        <Abstract>
                            <AbstractText>No PMID article.</AbstractText>
                        </Abstract>
                    </Article>
                </MedlineCitation>
            </PubmedArticle>
    ...
    def test_search_cache_hit(self) -> None:
        ...
        miner = PubMedMiner(enable_cache=True)
        miner._search_cache["test query:10"] = ["111", "222", "333"]
        result = miner.search("test query", max_results=10)
        assert result == ["111", "222", "333"]

    def test_search_cache_operations(self) -> None:
        """cache size and clear work correctly."""
        miner = PubMedMiner(enable_cache=True)
        miner._search_cache["a:1"] = ["1"]
        miner._search_cache["b:2"] = ["2"]
        assert miner.get_cache_size() == 2
        miner.clear_cache()
        assert miner.get_cache_size() == 0


class TestArXivMinerParsing:
    """Cover ArXiv XML parsing edge cases."""

    def test_parse_arxiv_entry_no_title(self) -> None:
        """_parse_arxiv_entry returns None when title is empty."""
        import xml.etree.ElementTree as ET

        ns = {"arxiv": "http://www.w3.org/2005/Atom"}
        entry_xml = '<entry xmlns="http://www.w3.org/2005/Atom"><title></title></entry>'
        entry = ET.fromstring(entry_xml)
        miner = ArXivMiner()
        result = miner._parse_arxiv_entry(entry, ns)
        assert result is None

    def test_parse_arxiv_entry_full(self) -> None:
        """_parse_arxiv_entry parses a complete entry."""
        import xml.etree.ElementTree as ET
        ...
        entry_xml = """<entry xmlns="http://www.w3.org/2005/Atom">
            ...
            <summary>Study of ant behavior.</summary>
        ...
        pub = miner._parse_arxiv_entry(entry, ns)
        assert pub is not None
        assert pub.title == "Ant Colony Optimization"
        assert len(pub.authors) == 2
        assert pub.year == 2023
