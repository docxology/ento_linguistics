...
Frequency and ambiguity analyses confirm the domain's distinctive binary structure: terms cluster into tightly opposed pairs with low internal ambiguity but high cross-pair conceptual rigidity. Full per-domain frequency and word-formation patterns are shown in Figures \ref{fig:domain_overview_grid} and \ref{fig:domain_patterns_grid}.

## Kin \& Relatedness

Kin and Relatedness terminology exhibits the lowest mean semantic entropy (0.25 bits) and a web-like network architecture reflecting the complex, non-intuitive relatedness structures of haplodiploid societies (Figure \ref{fig:domain_overview_grid}). The central tension is between human bilateral kinship models—where siblings share $r = 0.5$—and the haplodiploidy-specific asymmetry where full sisters share $r = 0.75$ but sisters relate to brothers at only $r = 0.25$. When researchers describe colony members as "sisters," the term imports an assumption of symmetry that masks the very asymmetry on which inclusive fitness theory depends.
...
Researchers should become intensely aware of how their terminological choices import assumptions. The domain contrasts that the statistical battery supports—the elevated semantic entropy of Economics terminology and the dense bridging vocabulary of Power \& Labor (139 bridging terms)—trace exactly the contours where linguistic precision would most improve scientific communication. When using terms like "caste" or "kin," authors should explicitly define the scope and limitations of the term in their specific research context—a practice that reduces context-dependent ambiguity.
...
|---------|-------|---------|
| `biological_individuality` | 568 | Unit of Individuality |
| `social_organization` | 556 | Power & Labor; Behavior & Identity |
| `reproductive_biology` | 256 | Sex & Reproduction |
| `kinship_systems` | 311 | Kin & Relatedness |
| `resource_economics` | 96 | Economics |
| `behavioral_ecology` | 376 | Behavior & Identity; Economics |
...
\texttt{src/pipeline/fulltext\_pipeline.py}): 0.0170
...
full-text layer, with 1046 versus
...
2460 ant/myrmecology documents harvested from the BHL
...
monotonically from 0.0836 per 10k
tokens in 1850-1899 (1013 documents) to
0.1130 in 1900-1949
(1247 documents) and
0.2530 in 1950-1970
(200 documents) — an intensification
...
rises from 0.6583 to
0.7938 to
1.4608 per 10k tokens across the
