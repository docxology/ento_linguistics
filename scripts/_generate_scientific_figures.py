...
try:
    ...
    class _FallbackMetrics:
        """Fallback metrics when infrastructure is not available."""
        duration = 0.0
    ...
    class PerformanceMonitor:
        """Fallback PerformanceMonitor when infrastructure is not available."""
        def start(self): pass
        def update_memory(self): pass
        def stop(self): return _FallbackMetrics()
...
def insert_figures_into_manuscript(figure_labels: list[str]) -> None:
    ...
    target_file = manuscript_dir / "04a_corpus_and_networks.md"
