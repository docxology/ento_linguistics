...
```bash
sudo tlmgr install multirow cleveref newunicodechar
```

(Note: `bm` and `subcaption` are already included in BasicTeX as part of the `tools` and `caption` packages. The `doi` package is **not used**; DOIs are rendered as inline hyperlinks via `\providecommand{\doi}[1]{\href{...}{doi:#1}}`—see *Bibliography DOI policy* below.)
...
% natbib: loaded by Pandoc’s LaTeX template; `[round,comma,sort&compress]` is injected in _render_pdf_override.py post-process
...
%
% Bibliography DOI policy:
%   DOIs are rendered as inline hyperlinks via \providecommand{\doi}.
%   The doi.sty package is NOT used (it produces footnotes).
%   To suppress DOIs entirely, replace with:
